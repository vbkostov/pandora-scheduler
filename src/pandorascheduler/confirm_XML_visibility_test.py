...
def parse_xml(file_path):
    ...
    for visit in root.findall('Visit'):
        visit_id = visit.find('ID').text
        
        for sequence in visit.findall('Observation_Sequence'):
            sequence_id = sequence.find('ID').text
            start_time = sequence.find('.//Timing/Start').text
            stop_time = sequence.find('.//Timing/Stop').text
            
            visits.append({
                'Task': f"Visit {visit_id} - Sequence {sequence_id}",
                'Start': datetime.strptime(start_time, '%Y-%m-%dT%H:%M:%SZ'),
                'Finish': datetime.strptime(stop_time, '%Y-%m-%dT%H:%M:%SZ'),
                'Visit': visit_id
            })
...
def create_gantt_chart(df):
    fig = ff.create_gantt(df, index_col='Visit', show_colorbar=True, group_tasks=True)
    fig.update_layout(title='Visit and Sequence Gantt Chart',
                      xaxis_title='Time',
                      yaxis_title='Visit',
                      height=400,
                      width=800)
...
def save_gantt_chart_as_png(fig, output_path):
    fig.write_image(output_path, scale=2)  # scale=2 for higher resolution


# Main execution
if __name__ == "__main__":
    xml_file_path = f'{PACKAGEDIR}/data/Pandora_science_calendar.xml'
    output_dir = PACKAGEDIR + '/data/confirm_visibility'
    output_png_path = os.path.join(output_dir, f'TMP_visit_visibility.png')

    df = parse_xml(xml_file_path)
    gantt_chart = create_gantt_chart(df)
    
    # Save the chart as PNG
    save_gantt_chart_as_png(gantt_chart, output_png_path)
    
    print(f"Gantt chart saved as PNG: {output_png_path}")
    
    # Optionally, still show the interactive chart in the browser
    gantt_chart.show()
